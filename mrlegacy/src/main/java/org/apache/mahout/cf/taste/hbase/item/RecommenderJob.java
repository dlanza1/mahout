...
public final class RecommenderJob extends AbstractJob {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (shouldRunNextPhase(parsedArgs, currentPhase)) {
      ...
      if (filterFile != null) {
        setS3SafeCombinedInputPath(aggregateAndRecommend_hb, getTempPath(), partialMultiplyPath, explicitFilterPath);
      }
      setIOSort(aggregateAndRecommend_hb);
      aggregateAndRecommendConf_hb.set(AggregateAndRecommendReducer.ITEMID_INDEX_PATH,
              new Path(prepPath, PreparePreferenceMatrixJob.ITEMID_INDEX).toString());
      
      aggregateAndRecommendConf_hb.setInt(AggregateAndRecommendReducer.NUM_RECOMMENDATIONS, numRecommendations);
